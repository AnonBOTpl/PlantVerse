...
1. **Original (FloraSense):** Fast, local classification using the FloraSense (Hugging Face) model, ideal for quick species labeling.
2. **Advanced (Gemini AI):** Deep multimodal analysis powered by Google's latest Gemini 2.0 Flash model.
3. **Hybrid (Verification):** The most accurate mode. The local model provides an initial prediction which is then analyzed, verified, or corrected by Gemini AI.
...
**Installation Steps:**
1. Clone the repository: Use the **git clone** command followed by your fork's URL.
2. Enter the directory: Use the **cd PlantVerse** command.
3. Install dependencies: Run **pip install -r requirements.txt** in your terminal.
4. Launch the application: Run **streamlit run app.py** to start the web interface.
